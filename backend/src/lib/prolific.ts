...
export interface ProlificClaims {
  PROLIFIC_PID: string;
  STUDY_ID: string;
  SESSION_ID: string;
}
...
interface JwtPayload {
  ...
  aud?: string | string[];
  exp?: number;
  prolific?: Partial<ProlificClaims>;
  PROLIFIC_PID?: string;
  STUDY_ID?: string;
  SESSION_ID?: string;
}
...
function assertProlificIds(claims: Partial<ProlificClaims>): ProlificClaims {
...
function extractClaims(payload: JwtPayload): ProlificClaims {
  if (payload.prolific) return assertProlificIds(payload.prolific);
  return assertProlificIds(payload);
}

function normalizeAud(aud: string | string[] | undefined): string[] {
  if (!aud) return [];
  return Array.isArray(aud) ? aud : [aud];
}

function originOf(urlLike: string): string {
  try {
...
async function verifyRs256(headerB64: string, payloadB64: string, signatureB64: string, jwk: JsonWebKey): Promise<boolean> {
  ...
  return crypto.subtle.verify("RSASSA-PKCS1-v1_5", cryptoKey, signature.buffer as ArrayBuffer, data);
}
...
async function fetchJwks(): Promise<JsonWebKeySet> {
  ...
  if (apiToken.length > 0) headers.authorization = `Token ${apiToken}`;

  const res = await fetch(url, { headers });
  if (!res.ok) throw new Error(`Failed to fetch Prolific JWKS (${res.status})`);
...
function validateStandardClaims(payload: JwtPayload) {
  ...
  if (expectedAud.length > 0) {
    ...
    if (!audMatch) throw new Error("Invalid Prolific token audience");
  }
}

function allowDevToken(): boolean {
  return envOr("ALLOW_DEV_JWT", "false").toLowerCase() === "true";
}

function allowUnsignedUrlParams(): boolean {
  return envOr("PROLIFIC_ALLOW_UNSIGNED_PARAMS", "true").toLowerCase() === "true";
}

function parseDevJsonToken(token: string): ProlificClaims {
  const parsed = JSON.parse(token) as Partial<ProlificClaims>;
  return assertProlificIds(parsed);
}

export async function verifyProlificSecuredUrlJwt(token: string): Promise<ProlificClaims> {
  ...
  if (parts.length !== 3) throw new Error("Invalid Prolific token format");
  const [headerB64, payloadB64, signatureB64] = parts;
  const header = decodeJson<JwtHeader>(headerB64);
  if (header.alg !== "RS256") throw new Error("Unsupported Prolific token algorithm");
  if (!header.kid) throw new Error("Missing Prolific token kid");

  const jwks = await fetchJwks();
  const jwk = jwks.keys.find((k) => ((k as { kid?: string }).kid === header.kid) && k.kty === "RSA");
  if (!jwk) throw new Error("Unable to find Prolific signing key");

  const verified = await verifyRs256(headerB64, payloadB64, signatureB64, jwk);
  if (!verified) throw new Error("Invalid Prolific token signature");
